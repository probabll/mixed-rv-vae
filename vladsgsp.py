...
from gaussiansp import GaussianSparsemax as GSP
...
def main():
    ...
    std = torch.tensor([.2, .1, .3]).unsqueeze(0)

    gs = GaussianSparsemax(loc=mu, scale=std)

    gs = gs.expand((10, 1))
    Y = gs.sample()  #sample_shape=(10,))
    print(Y)
    print(Y.shape)
    print("Closed-form log_prob (1-D MC 100 samples)")
    print(gs.log_prob(Y, n_samples=100))
    print("Closed-form log_prob (1-D MC 100000 samples)")
    v = 0
    for _ in range(10):
        v += gs.log_prob(Y, n_samples=10)
    print(v/10)
    print("Closed-form log_prob (1-D MC 100000 samples)")
    v = 0
    for _ in range(10):
        v += gs.log_prob(Y, n_samples=10)
    print(v/10)
    #print("Compare against IS, K-D 1 mil samples:")
    #print(gs.log_prob_IS(Y, n_samples=1000000))
    #print(torch.tensor([gs.log_prob_IS(y, n_samples=1000000) for y in Y]))
